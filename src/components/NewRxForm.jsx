...
export function NewRxForm({ onCreated }) {
  ...
  const addDrug = (drug) => {
    if (items.find(i => i.drug_id === drug.id)) return;
    setItems([...items, {
      drug_id: drug.id,
      nama: `${drug.nama_dagang} ${drug.kekuatan_dosis}`,
      quantity_prescribed: drugQty,
          ...
          <div className="form-group">
            ...
            <div className="drug-add-row">
              ...
              <div className="form-group" style={{ flex: 0, minWidth: 70 }}>
                <input className="form-input" type="number" min={1} value={drugQty} onChange={e => setDrugQty(Number(e.target.value))} style={{ width: 70 }} />
              </div>
            </div>
            {drugResults.length > 0 && (
              <div style={{ maxHeight: 150, overflowY: 'auto', marginTop: 4, background: 'var(--bg-primary)', borderRadius: 6, border: '1px solid var(--border)' }}>
                {drugResults.map(d => (
                  <div key={d.id} className="search-result" onClick={() => addDrug(d)} style={{ padding: '6px 10px', fontSize: 13 }}>
                    <div className="search-result__info">
                      <span className="search-result__name">{d.nama_dagang} {d.kekuatan_dosis}</span>
                      <span className="search-result__detail" style={{ marginLeft: 8 }}>Stok: {d.stok_saat_ini}</span>
                    </div>
                  </div>
                ))}
              </div>
